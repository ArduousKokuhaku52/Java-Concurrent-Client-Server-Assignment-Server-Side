...
public class ReceiverServer extends Thread{
	 ...
	 ********************************************************/
	public ReceiverServer(ArrayList<Message> messages, int port) {
		this.messages = messages;
		try {
			this.server = new ServerSocket(port);
			System.out.println("Receiving Server started...");
		}
		catch(IOException e) {
	 ...
	 * 	and starts ReceiverHandler to process the client. Runs
	...
	public void run() {
		while(true) {	//runs in infinite loop until manually terminated
			try {
				...
				ReceiverHandler handler = new ReceiverHandler(this.messages, client);
